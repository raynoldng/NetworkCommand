...
public class GameLevel extends ManagedGameScene implements IOnSceneTouchListener {
	...
	public volatile List<Camp> mCamps = Collections.synchronizedList(new ArrayList<Camp>());
	...
	public void onLoadLevel() {
		...
		this.addLoadingStep(new LoadingRunnable("Loading game textures", this) {
			...
			public void onLoad() {
				...
				// set the zoom factor, dependent on map size and the camera dimension
				...
				for(final CampDef curCamp : GameLevel.this.mLevelDef.mCamps) {
					mCamps.add(new Camp(curCamp.mX, curCamp.mY, curCamp.mSize, curCamp.color, GameLevel.this));
					//mCamps.add(camp);
				}
				...
				sortChildren();	// so that the camps appear above the lines (edges)
			}
		});
